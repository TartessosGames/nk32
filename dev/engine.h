...
void espera_activa (int espera) {
   ...
   void process_tile(void) {
    #if !defined DEACTIVATE_KEYS && MAX_CERROJOS > 0
        // Procesar cerrojos (tile 10)
        if (qtile(x0, y0) == 10) {
            for (gpit = 0; gpit < MAX_CERROJOS; ++gpit) {
                if (cerrojos[gpit].x == x0 && cerrojos[gpit].y == y0 && cerrojos[gpit].np == n_pant) {
                    if (p_keys > 0) {
                        // Eliminar cerrojo detectado
                        cerrojos[gpit].st = 0;
                        _x = x0; _y = y0; _t = 0; _n = 0; update_tile();

                        // Eliminar cerrojo inferior y desactivarlo en cerrojos[]
                        _x = x0; _y = y0 + 1; _t = 0; _n = 0; update_tile();
                        for (gpjt = 0; gpjt < MAX_CERROJOS; ++gpjt) {  // Usar gpjt para evitar conflicto con gpit
                    ...
                    return;  // Salir para no procesar como desplazable
                }
            }
        }
    #endif
            ...
            #endif
            {
                    ...
                    p_disparando = 1;
                #endif
                // Procesar desplazables (tile 10 o 14)
